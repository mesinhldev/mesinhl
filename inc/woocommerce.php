...
add_action( 'after_setup_theme', 'mesinhl_woocommerce_support' );
if ( ! function_exists( 'mesinhl_woocommerce_support' ) ) {
	/**
	 * Declares WooCommerce theme support.
	 */
	function mesinhl_woocommerce_support() {
		...
		add_filter( 'woocommerce_form_field_args', 'mesinhl_wc_form_field_args', 10, 3 );
		add_filter( 'woocommerce_form_field_radio', 'mesinhl_wc_form_field_radio', 10, 4 );
		add_filter( 'woocommerce_quantity_input_classes', 'mesinhl_quantity_input_classes' );
		add_filter( 'woocommerce_loop_add_to_cart_args', 'mesinhl_loop_add_to_cart_args' );

		// Wrap the add-to-cart link in `div.add-to-cart-container`.
		add_filter( 'woocommerce_loop_add_to_cart_link', 'mesinhl_loop_add_to_cart_link' );

		// Add Bootstrap classes to account navigation.
		add_filter( 'woocommerce_account_menu_item_classes', 'mesinhl_account_menu_item_classes' );
	}
}
...
add_action( 'woocommerce_before_main_content', 'mesinhl_woocommerce_wrapper_start', 10 );
add_action( 'woocommerce_after_main_content', 'mesinhl_woocommerce_wrapper_end', 10 );

if ( ! function_exists( 'mesinhl_woocommerce_wrapper_start' ) ) {
	/**
	 * Display the theme specific start of the page wrapper.
	 */
	function mesinhl_woocommerce_wrapper_start() {
		$container = get_theme_mod( 'mesinhl_container_type' );
		if ( false === $container ) {
			$container = '';
		}
...
if ( ! function_exists( 'mesinhl_woocommerce_wrapper_end' ) ) {
	/**
	 * Display the theme specific end of the page wrapper.
	 */
	function mesinhl_woocommerce_wrapper_end() {
		echo '</main>';
...
if ( ! function_exists( 'mesinhl_wc_form_field_args' ) ) {
	/**
	 ...
	 */
	function mesinhl_wc_form_field_args( $args, $key, $value ) {
		$bootstrap4 = 'bootstrap4' === get_theme_mod( 'mesinhl_bootstrap_version', 'bootstrap4' );
...
if ( ! function_exists( 'mesinhl_wc_form_field_radio' ) ) {
	/**
	 ...
	 * `mesinhl_wc_form_field_args()` that is meant for input labels only.
	 *
	 ...
	 * @return string
	 */
	function mesinhl_wc_form_field_radio( $field, $key, $args, $value ) {
...
if ( ! function_exists( 'mesinhl_quantity_input_classes' ) ) {
	/**
	 ...
	 * @return array
	 */
	function mesinhl_quantity_input_classes( $classes ) {
		$classes[] = 'form-control';
		return $classes;
	}
}

if ( ! function_exists( 'mesinhl_loop_add_to_cart_link' ) ) {
	/**
	 ...
	 */
	function mesinhl_loop_add_to_cart_link( $html ) {
		return '<div class="add-to-cart-container">' . $html . '</div>';
	}
}

if ( ! function_exists( 'mesinhl_loop_add_to_cart_args' ) ) {
	/**
	 ...
	 */
	function mesinhl_loop_add_to_cart_args( $args ) {
...
if ( ! function_exists( 'mesinhl_account_menu_item_classes' ) ) {
	/**
	 ...
	 */
	function mesinhl_account_menu_item_classes( $classes ) {
